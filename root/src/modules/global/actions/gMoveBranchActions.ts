...
Do you want to discard those changes and move a branch?`;
        ...
        return gMoveBranchActions.selectMoveOption(
            state,
            option);
    },
    ...
    selectMoveOption: (
        ...
        return gMoveBranchActions.completeOptionSelection(state);
    },
    ...
    completeOptionSelection: (state: IState): IStateAnyArray => {
        ...
        state.lens.nodeTab.stageBehaviour = gStageCode.buildMoveBranchStages(branchTask);

        return gBranchTaskActions.completeSelectionAndNext(state);
    }
};

export default gMoveBranchActions;
