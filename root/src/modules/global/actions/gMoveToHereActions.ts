...
const gMoveToHereActions = {
    ...
    setUp: (
        ...
        if (U.isNullOrWhiteSpace(dirty) === false) {
            ...
            const message = `${dirty}.
Do you want to discard those changes and move a branch to this discussion?`;

            // Then need the user to confirm they want to discard the changes
            gDialogueCode.buildDialogue(
                state,
                DialogueType.Confirm,
                gMoveToHereActions.selectTargetDiscussion,
                DelegateType.Action,
        ...
        return gMoveToHereActions.selectTargetDiscussion(
            state,
            target);
    },
    ...
    completeTargetSelection: (state: IState): IStateAnyArray => {
        ...
        state.lens.nodeTab.stageBehaviour = gStageCode.buildMoveToHereStages(branchTask);

        return gBranchTaskActions.completeSelectionAndNext(state);
    }
};

export default gMoveToHereActions;
